...
#' With vs. without functional annotation
cluster.sizes <- list()
qtl.setOption(cluster.cor.threshold = 0.2)
qtl.setOption(standard.deviation.gauss = 3000)
qtl.setOption(absolute.distance.cutoff=500000)
qtl.setOption(use.segmentation=FALSE)
params <- c(TRUE,FALSE)
cluster.sizes <- foreach(param=params,.combine="cbind")%dopar%{
  qtl.setOption(use.functional.annotation = param)
  cluster.sizes.param <- unlist(lapply(1:1000,function(foo){
    sel.chr <- sample(1:23,1)
    anno.chr <- anno.genome[[sel.chr]]
    sel.cpgs <- sample(1:(length(anno.chr)-1000),1)
...
to.plot <- data.frame(params=c(rep(params[1],nrow(cluster.sizes)),rep(params[2],nrow(cluster.sizes))),cluster_size=c(cluster.sizes[,1],cluster.sizes[,2]))
plot <- ggplot(to.plot,aes(x=params,y=cluster_size))+
